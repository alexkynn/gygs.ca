...
async function generateMasterResponse(question, mode = 'teaser') {
    try {
        ...
        const prompt = `
你是一位精通中西命理的 AI 戰略家。請根據以下精確的排盤數據，撰寫一份約 3000 字、結構完整、資訊密度極高的命理分析報告。

【寫作規範與一致性原則】（請嚴格遵守）：
1. 完整性：必須「一氣呵成」寫完全部六大章節（從壹到陸），絕對不能中途停止或斷尾！請盡情展開詳細論述。
2. 運勢評分標準：未來 10 年運勢，請嚴格依據大運流年五行與本命喜忌進行機械化對應。遇喜用神之年固定給 85-95 分，忌神年固定給 50-65 分。每年評語格式必須統一為：「年份 | ██████░░░░ (分數) - [運勢關鍵字] 具體事件預測」。
3. 防幻覺機制：紫微斗數「絕對不允許」出現 <FactData> 中未列出的星曜。四柱神煞請嚴格依據學理推導最基礎且必定的神煞（如：戌日見魁罡），切勿隨機增加。
4. 調候統一論述：在調候分析中，必須將原局五行狀態與缺失用神融合。請統一得出此核心結論：「不可單打獨鬥，必須借力市場資源與資本槓桿，以市場實踐清洗體制腐朽」。
5. 排版限制：僅允許使用 Markdown 的標題 (##, ###) 與列表。嚴禁使用 HTML、LaTeX (嚴禁 $$ 符號) 或表格。
...
## 參、四柱神煞詳解與調候樞紐
### 一、 調候樞紐分析
（精準點出調候用神，並嚴格置入【規範4】所要求的現代商業策略結論。）
### 二、 四柱神煞嚴謹推算與現代解讀
（必須分列「1. 年柱」、「2. 月柱」、「3. 日柱」、「4. 時柱」，逐一解釋其上的關鍵神煞對命運的影響。）

## 肆、紫微斗數全景與核心宮位深度解析
（必須包含並詳細解析以下四大宮位，缺一不可：「財帛宮」、「官祿宮」、「遷移宮」、「夫妻宮」。
【格局鑑定重點】：從文獻參考中，比對紫微斗數 31 種標準格局，明確鑑定命主的 12 宮位星曜組合符合哪些特殊格局，並深入解析。）

## 伍、未來 10 年運勢推演
（請嚴格遵守【規範2】，使用純文字長條圖繪製未來 10 年運勢，請連續寫滿 10 年，畫完後進行大勢推演。）

## 陸、大師戰略行動指南
（請給出務實的避險與進攻策略。寫完此段即完成報告。）
...
${contexts}
        ...
        const model = genAI.getGenerativeModel({ 
            ...
            // 🟢 解除貪婪解碼鎖定，給予 AI 呼吸空間，確保 3000 字能順暢生成不斷尾
            generationConfig: {
                temperature: 0.3,
