...
【排版與精簡鐵律】必須使用現代層級編號系統 (如 1. -> 1.1 -> 1.1.1)。語言必須極度精煉，去蕪存菁。行動指導強制使用列點式（Bullet points），每點不超過 80 字，以最高密度傳遞信息。
...
### 2.2 五行喜忌深度剖析與行為套利
（【防幻覺鐵律】：必須完全採用 <FactData> 中提供的「系統判定日元強度」、「絕對最喜用神」與「絕對最忌五行」，並簡述其強弱分數邏輯。【必須給出】該喜用五行在現代職場對應的具體「行為套利」策略，強制以精簡列點呈現。）
### 2.3 專屬開運密碼與產業佈局
...
4. 【排版與精簡鐵律】：所有策略與建議，必須強制使用精簡的列點式（Bullet points），每點不超過 80 字。
...
### 3.3 四柱神煞與現代套利
（挑選命局中 3-5 個最具代表性的神煞，參考古籍定義，將其轉化為現代商業與避險的槓桿工具。例如將「天乙貴人」解讀為機構背書，「驛馬」解讀為跨國市場等，給出極度務實的高密度套利解讀。）
...
4. 【篇幅控管與排版鐵律】：為了確保運算完整，每個宮位的總字數必須嚴格控制在 600-800 字以內。在撰寫「現代商業套利指導」時，必須強制使用精簡的列點式（Bullet points），每點不超過 80 字。以最高密度的語言傳遞最大的信息量。
5. 【強制完賽指令】：請自行調控字數與篇幅，絕對不可提早截斷！你必須確保「#### 4.4 夫妻宮」完整產出後才可停止。請為下半部保留運算空間。
...
（【格式與精簡鐵律】：禁用區塊符號。必須從 ${currentDateStr} 所在的今年開始算起，連續推演 10 年。未來十年每一年都「必須」將年份加粗（如 **2026年**），並加上括號標註「週期定調（如：【爆發期】、【防守期】等）」！推演內容請保持高密度精簡，每年的分析與建議請列點說明，並控制在 100 字以內。）
（【極度重要】：明確標示出這十年中哪一年是極度高危的「黑天鵝年（Black Swan）」，並給出防守策略。）
（格式範例：**2026年** | 80分 | 【破局爆發期】
- 事業轉折：事業出現重大轉折，利於高風險進攻。
- 防守策略：秋季需防範合約陷阱。）
...
### 6.3 進攻與避險戰略
（【排版與精簡鐵律】：條列 3 項極度務實的現代行動建議。必須強制使用 #### 6.3.1, #### 6.3.2, #### 6.3.3 作為子標題。內容必須強制使用精簡的列點式（Bullet points），每點不超過 80 字。）
### 6.4 五行環境與心理調校
（結合最喜用神，給出環境風水、穿搭顏色或心態調整建議。同樣強制使用列點式說明。）
...
4. 【高密度精簡鐵律】：語言必須極度精煉，去蕪存菁。行動策略強制使用精簡列點式（每點不超過 80 字）。
...
### 7.4 專屬溝通與執行套利策略
（針對該 MBTI 的資訊處理習慣，給出量身打造的職場溝通與決策執行手冊。強制使用列點式說明，每點不超過 80 字。）
`;
}
