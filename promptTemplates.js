...
【防幻覺與語言鐵律】絕對禁止在文中夾雜漢語拼音或英文字母（如 Jia-Yin），僅能使用純中文干支。嚴禁自行推算或捏造大運干支，必須以 <FactData> 提供的數據為準。
【防套話與去油膩鐵律】在給出「行為套利」策略時，絕對禁止使用「建立SOP」、「輕資產運營」、「資產防火牆」等千篇一律的農場文套話。必須根據命局給出具體、犀利且具唯一性的指導。
【節省 Token 指令】你「只能」撰寫第 1 至第 2 大段。寫完 2.3 後立刻停止。絕對禁止任何開場白或結語！
【排版鐵律】必須使用現代層級編號系統 (如 1. -> 1.1 -> 1.1.1)。你的解讀必須極度深湛、詳盡。
...
4. 【防幻覺與語言鐵律】：絕對禁止在輸出中夾帶漢語拼音（如 Jia-Yin），必須使用純中文干支。嚴禁自行捏造大運干支。
5. 【防套話與去油膩鐵律】：解析神煞與現代套利時，絕對禁止使用「建立SOP」、「資產隔離」、「輕資產運營」、「知識付費」等千篇一律的農場文套話。你的商業建議必須高度客製化，針對命主獨特的干支五行給出犀利且具『唯一性』的指導，嚴禁放諸四海皆準的廢話。
...
3. 【重點聚焦鐵律】：絕對禁止像流水帳一樣羅列所有小星！必須運用「二八法則」，只挑選該宮位中「最核心的 1-2 顆主星」與「最具破壞力/爆發力的 1-2 顆煞星或吉星」進行深度剖析，忽略無關緊要的雜曜。
4. 【篇幅控管與排版鐵律】：為了確保運算完整，每個宮位的總字數必須嚴格控制在 600-800 字以內。在撰寫「現代商業套利指導」時，必須強制使用精簡的列點式（Bullet points），每點不超過 100 字。以最高密度的語言傳遞最大的信息量。
5. 【防幻覺與語言鐵律】：絕對禁止輸出任何漢語拼音，必須使用純中文干支。嚴禁自行捏造大運。
6. 【防套話與去油膩鐵律】：在撰寫每個宮位的「現代商業套利指導」時，絕對禁止重複使用「建立SOP」、「資產防火牆」、「輕資產營運」、「知識付費」等罐頭商業術語。必須根據該宮位的特定星曜組合，給出具有『唯一性』與『針對性』的破局策略。
7. 【強制完賽指令】：請自行調控字數與篇幅，絕對不可提早截斷！你必須確保「#### 4.4 夫妻宮」完整產出後才可停止。請為下半部保留運算空間。
...
5. 【防幻覺與語言鐵律】：絕對禁止輸出任何漢語拼音，必須使用純中文干支。嚴禁自行捏造大運。
6. 【防套話與去油膩鐵律】：在撰寫每個宮位的「現代商業套利指導」時，絕對禁止重複使用「建立SOP」、「資產防火牆」、「輕資產營運」、「知識付費」等罐頭商業術語。必須根據該宮位的特定星曜組合，給出具有『唯一性』與『針對性』的破局策略。
7. 【強制完賽指令】：請自行調控字數與篇幅，絕對不可提早截斷！你必須確保「#### 4.8 遷移宮」完整產出後才可停止。
...
5. 【防幻覺與語言鐵律】：絕對禁止輸出任何漢語拼音，必須使用純中文干支。嚴禁自行捏造大運。
6. 【防套話與去油膩鐵律】：在撰寫每個宮位的「現代商業套利指導」時，絕對禁止重複使用「建立SOP」、「資產防火牆」、「輕資產營運」、「知識付費」等罐頭商業術語。必須根據該宮位的特定星曜組合，給出具有『唯一性』與『針對性』的破局策略。
7. 【強制完賽指令】：請自行調控字數與篇幅，絕對不可提早截斷！你必須確保「#### 4.12 父母宮」完整產出後才可停止。
...
4. 【防幻覺與語言鐵律】：絕對禁止在輸出中夾帶任何漢語拼音或英文字母（如 Jia-Yin, Yi-Mao），必須純全使用中文干支（如 甲寅、乙卯）。絕對禁止自行推算或捏造大運干支！必須完全依賴 <FactData> 提供的數據，嚴禁張冠李戴或產生邏輯矛盾。

請「嚴格按照以下結構」接續撰寫：

## 5. 未來 10 年運勢推演（含黑天鵝風險警示）
（【格式鐵律】：禁用區塊符號。必須從 ${currentDateStr} 所在的今年開始算起，連續推演 10 年。未來十年每一年都「必須」將年份加粗（如 **2026年**），並加上括號標註「週期定調（如：【爆發期】、【防守期】等）」！每一年的推演必須極度詳盡深湛。）
（【極度重要】：明確標示出這十年中哪一年是極度高危的「黑天鵝年（Black Swan）」，並給出防守策略。對於「黑天鵝年」以及運勢最頂峰的「黃金爆發年」，【必須額外提供】高危/高光月份的精準流月解析。）
（格式範例：**2026年** | 80分 | 【破局爆發期】事業出現重大轉折...）
...
3. 【防幻覺與語言鐵律】：絕對禁止輸出任何漢語拼音，必須使用純中文干支。嚴禁自行捏造大運。
4. 【防套話與去油膩鐵律】：絕對禁止使用「建立SOP」、「資產隔離」、「輕資產運營」等罐頭商業術語。你的行動建議必須極度客製化，針對提問與命格給出犀利且具『唯一性』的破局方案。
...
3. 【交叉驗證指令】：Section 7 的心理推論必須與前文的「絕對最喜用神」與「絕對最忌五行」保持 100% 數據一致，絕不可產生矛盾。你的推演必須極度深湛。
4. 【防幻覺與語言鐵律】：絕對禁止輸出任何漢語拼音，必須使用純中文干支。嚴禁自行捏造大運。
5. 【防套話與去油膩鐵律】：在給出溝通與執行手冊時，絕對禁止使用「建立SOP」、「資產隔離」等罐頭術語。必須結合 MBTI 認知功能與命理，給出極具深度的獨家指導。
